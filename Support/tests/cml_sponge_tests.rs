...
use catwalk::cml_sponge::{cipher_init, cipher_init_r, keystream, keystream_r,
                               encrypt_in_place, decrypt_in_place,
                               absorb_aad, aead_encrypt_chunk, aead_decrypt_chunk, aead_finalize};
...
fn aead_tag_changes_with_aad() {
    ...
    assert_ne!(tag_with_aad_a, tag_with_aad_b,
...
fn aead_tag_changes_with_ciphertext_tamper() {
    ...
    let tampered_tag = {
        ...
        aead_decrypt_chunk(&mut s, &mut buf);
        aead_finalize(&mut s)
    };
...
fn aead_encrypt_decrypt_roundtrip_direct() {
    ...
    let (recovered, dec_tag) = {
        ...
        aead_decrypt_chunk(&mut s, &mut pt);
        let tag = aead_finalize(&mut s);
        (pt, tag)
    };
...
fn aead_absorb_aad_domain_separation() {
    ...
    assert_ne!(tag_aad_only, tag_ct_only,
...
fn arnold_cat_map_not_involution() {
    ...
    assert_ne!((x2, y2), (x, y), "cat_map applied twice must not return original (not an involution)");
}
...
fn arnold_cat_map_area_preserving() {
    ...
    assert_eq!(x_rec, x, "Cat Map inverse (area-preserving check) must recover x");
    assert_eq!(y_rec, y, "Cat Map inverse (area-preserving check) must recover y");
}
