...
fn main() {
    ...
    let mut seed_material = b"chaotic keystream statistical evaluation seed v5".to_vec();
    seed_material.push(seed_index);

    let key = blake3::derive_key("au79-crypto.practrand.test.key.v5", &seed_material);
    let nonce = [0u8; 16];
